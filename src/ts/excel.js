...
export function toMarkdownTable(rows) {
    if (!rows || rows.length === 0)
        return '';
    const lines = rows.map((row) => '| ' + row.map((c) => String(c ?? '').replace(/\|/g, '\\|')).join(' | ') + ' |');
 ...
 */
export function renderTableHTML(rows, maxRows = 50, maxCols = 20) {
    if (!rows || rows.length === 0)
        return '';
 ...
 * Thống kê file
 */
export function getTableInfo(rows, maxRows = 50) {
