...
// PIPELINE
// ───────────────────────────────────────────
export function setPipelineStep(step, state) {
    for (let i = 1; i <= 4; i++) {
...
export function addThinkingIndicator(label) {
    removeWelcome();
...
export function addToolCard(results) {
    const msgs = getMessagesContainer();
    let rows = '';
...
// STATS
// ───────────────────────────────────────────
export function updateStats(calls, calcs, questions) {
    const el = (id) => document.getElementById(id);
    const c = el('stat-calls');
    const ca = el('stat-calcs');
    const q = el('stat-questions');
    if (c)
        c.textContent = String(calls);
    if (ca)
        ca.textContent = String(calcs);
    if (q)
        q.textContent = String(questions);
}
// ───────────────────────────────────────────
// INPUT
// ───────────────────────────────────────────
