...
let selectedSheets: Set<string> = new Set();
...
function toggleSheet(name: string): void {
  if (selectedSheets.has(name)) {
    ...
    if (selectedSheets.size <= 1) return;
...
function updateSheetTabStyles(): void {
...
function renderMergedPreview(): void {
  if (!state.excelData) return;
...
async function sendMessage(): Promise<void> {
  ...
  try {
      ...
      if (response.stopReason === 'tool_use' && response.toolCalls.length > 0) {
        ...
        if (seenEquations.has(currentEquations)) {
          ...
          if (stuckCount >= 1) {
            ...
            messages = [{ role: 'user' as const, content: forcePrompt }];
        ...
        if (state.provider === 'claude') {
          ...
          messages = geminiHistory as typeof messages;
        } else {
    ...
    if (!finalAnswer && allCalcResults.length > 0) {
      ...
      const forceMessages = [{ role: 'user' as const, content: forcePrompt }];
      try {
